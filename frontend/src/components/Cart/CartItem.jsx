...
const CartItem = ({ item }) => {
    ...
    // Misma lógica que en ProductCard para construir la URL de la imagen
        ...
        <Grid container alignItems="center" spacing={2} sx={{ mb: 2, pb: 2, borderBottom: '1px solid #eee' }}>
            ...
            <Grid item xs={6} sm={4}>
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <Typography variant="body1" sx={{ mx: 1, fontWeight: 'bold', userSelect: 'none' }}>
                        {item.cantidad}
                    </Typography>
                    <IconButton 
            ...
            <Grid item xs={6} sm={3} sx={{ textAlign: 'right', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 1 }}>
                <Typography variant="subtitle1" fontWeight="bold" color="primary.main">
                    ${(item.subtotal || item.cantidad * item.precio_actual).toLocaleString('es-AR', { minimumFractionDigits: 2 })}
                </Typography>
                <IconButton 
